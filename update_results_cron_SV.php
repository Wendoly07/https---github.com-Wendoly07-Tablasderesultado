...
// ─── Fetch JSON desde gamesdata.loto.sv ───────────────────────────────────
function fetchGamesData(): array {
    $ch = curl_init();
    curl_setopt_array($ch, [
        CURLOPT_URL            => 'https://gamesdata.loto.sv',
    ...
    if (!is_array($data)) {
        throw new Exception('JSON inválido recibido de gamesdata.loto.sv');
    }
    return $data;
}
...
function insertDraw(
    ...
    $stmtChk = sqlsrv_query(
        ...
        "SELECT COUNT(*) AS total
          ...
          WHERE pais = 'El Salvador' AND game_name = ? AND draw_number = ?",
        [$gameName, $drawNumber]
    );
    if ($stmtChk === false) {
    ...
    // Usar siempre el timestamp para draw_time (las claves del JSON no son confiables en SV)
...
// ─── Ejecución principal ───────────────────────────────────────────────────
try {
    echo "=== Revisión SV " . date('Y-m-d H:i:s') . " ===\n";
